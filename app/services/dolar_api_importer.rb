...
class DolarApiImporter
  API_URL = Rails.application.credentials.dolar_api_url
  ...
  def import
    response = fetch_data
    return unless response && response["Dolares"].is_a?(Array)

    response["Dolares"].each do |uf|
      ...
      uf_record = DolarValue.find_or_initialize_by(date: date)
